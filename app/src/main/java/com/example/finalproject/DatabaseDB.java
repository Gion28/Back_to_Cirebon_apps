...
public class DatabaseDB extends SQLiteOpenHelper {
    ...
    public boolean check(String username, String password){
        String[] column = {row_id};
        SQLiteDatabase db = getReadableDatabase();
        String select = row_username + "=?" + " and " + row_password + "=?";
        String[] selectArgs = {username,password};
        Cursor cursor = db.query(table_name, column, select, selectArgs, null,null, null);
        int hit = cursor.getCount();
        cursor.close();
        db.close();
